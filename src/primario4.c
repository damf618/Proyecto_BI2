...
{
	dprimario_t * aux = (dprimario_t *)TEC1CallbackGPIOParams;
	BaseType_t uartTXupdate = pdFALSE;
	tx_message TXMsg;
	// Si el contador es 0 -> puedo tomar un paquete de la cola
	if ( aux->uart1.messageTX.txCounter == 0)
	{
		if(( xQueueReceiveFromISR( aux->uart1.onTxQueue,( void * ) &TXMsg, &uartTXupdate) ))
		aux->uart1.messageTX.txLen = TXMsg.txLen;
	}

	if ( aux->uart1.messageTX.txCounter < aux->uart1.messageTX.txLen)
	{ 	// Transmision
		uartTxWrite(aux->uart1.Uart , aux->uart1.messageTX.dato_tx[aux->uart1.messageTX.txCounter]);
		aux->uart1.messageTX.txCounter++;
	}

	if ( aux->uart1.messageTX.txCounter == aux->uart1.messageTX.txLen)
	{
		uartTxWrite(aux->uart1.Uart, '\r');
		uartTxWrite(aux->uart1.Uart, '\n');
		uartCallbackClr(aux->uart1.Uart, UART_TRANSMITER_FREE); // Deshabilito la interrupcion de Tx
		aux->uart1.messageTX.txLen = 0;
		aux->uart1.messageTX.txCounter = 0;
	}

	// Evaluamos el pedido de cambio de contexto necesario
	portYIELD_FROM_ISR( uartTXupdate );
}
...
static char RTOS_Components_Init(dprimario_t * prim){
		...
		// ----- Queue to manage the UART message transmission
		prim->uart1.onTxQueue = xQueueCreate( 1, sizeof( prim->uart1.messageTX ) );
		if( prim->uart1.onTxQueue == NULL )
			status=QUEUE_ERROR;
...
bool_t primInit(dprimario_t * pPrimario)
{
	...
	// Set of a callback parameter rfor gpio events
