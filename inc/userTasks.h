...
#define SPI_WRITE_READ_DELAY 750	//Time difference betweeen SPI Write and SPI Read
 ...
 * and sends the Writing Command + current state.
 *
 * Task Name: "Server_SysW"
 * Priority: +2
 * Parameter: None
 * Trigger: Periodicity, 3s
...
/** Task to establish communication through SPI protocol to the NodeMCU to verify if the post
 * of the current state of the system to the firebase realtime database was possible. The entire
 *  task is consist in writing the reading command and read the response of the NodeMCU related
 *   to the execution of the updateof the server current state.
 *
 * Task Name: "Server_SysR"
 * Priority: +2
 * Parameter: None
 * Trigger: Periodicity, 3s
...
/** Task to reset the SPI Slave (NodeMCU) in case the communication with the device failed or the upload
 *  of the current state into the web server was not possible. It is based on the booting sequence for
 *  the NodeMCU, it is a one shot task generated by the Server_SysR task after three failed attempts.
 *
 * Task Name: "Reset_Slave"
 * Priority: +3
 * Parameter: None
 * Trigger: Sempahore Counter reaches 0.
